...
var clipboardAttr = "data-clipboard-text";

function setGpvUrl() {
  let generatedUrl =
    "https://app.gpv.gg/?" + $.param($("#url-form [name]").serializeObject());
  $genURL.attr(clipboardAttr, generatedUrl);
  $genURL.attr("title", generatedUrl);
}

function showGpvUrl() {
  $genURL.text($genURL.attr(clipboardAttr));
}
...
$genURL.on("click", function (c) {
  ...
  clipboard.on("error", function () {
    $genURL.text("Ctrl+C/Cmd+C to copy!");
    clipboard.destroy();
  });
});
...
$genURL.on("focusout mouseleave", function () {
  let msgText = "Click to copy URL (Hover to show)";
  if ($(this).text() != msgText) {
    setTimeout(() => $(this).text(msgText), 1000);
  }
});

$genURL.on("focusin mouseenter", () => {
  showGpvUrl();
});
...
$(".modal-container[id]:not(#modal-template)").each(function () {
  ...
  $(this)
    .find(".modal .minimenu a[href=" + targetId + "]")
    .closest("li")
    .addClass("selected");
});
  ...
  $.each(mappingObj.mapping, function (key, value) {
    function setButtonValues() {
      currentItem.find('[data-object]:not([data-object=""])').each(function () {
        ...
        var displayName =
          currentItem.find(".axes-config").prop("checked") ||
          currentItem.find("input[type=radio][value=dpad]").prop("checked")
            ? properNames[dataObject.choiceType] + " " + dataObject.choice
            : dataObject.choiceOperand
            ? properNames[dataObject.choiceType] +
              " " +
              dataObject.choice +
              " " +
              dataObject.choiceOperand
    ...
    if (value.choiceType) {
      ...
      currentItem
        .find("[data-button=positive]")
        .attr("data-object", JSON.stringify(dataObj));
    }
    if (value.targetType) {
      currentItem
        .find("select[name=" + value.targetType + "]")
        .val(value.target);
    }
    if (value.axesConfig) {
      ...
      currentItem
        .find("[data-button=low-value]")
        .attr("data-value", value.axesConfig.lowValue)
        .html(value.axesConfig.lowValue)
        .data("previous-value", value.axesConfig.lowValue);
      currentItem
        .find("[data-button=high-value]")
        .attr("data-value", value.axesConfig.highValue)
        .html(value.axesConfig.highValue)
        .data("previous-value", value.axesConfig.highValue);
    }
    switch (value.targetType) {
      ...
      case "axes":
        ...
        if (value.positive) {
          currentItem
            .find("[data-button=positive]")
            .attr("data-object", JSON.stringify(value.positive));
        }
        if (value.negative && value.negative.choiceType) {
          currentItem
            .find("[data-button=negative]")
            .attr("data-object", JSON.stringify(value.negative));
        }
        break;
      case "dpad":
        ...
        if (value.positions) {
          $.each(value.positions, function (pos, val) {
            currentItem
              .find("[data-button=" + pos + "]")
              .html(val)
              .data("previous-value", val)
              .attr("data-value", val);
          });
        }
        break;
...
function buttonCapture(jqThis, mType) {
  ...
  for (var b = 0; b < tester.SNAPSHOT.buttons.length; b++) {
    tester.SNAPSHOT.buttons[b] = $.extend(
      {},
      gamepadSupport.gamepadsRaw[basePlayer].buttons[b]
    );
  }
  function tidyUp() {
  ...
  jqThis.on("GamepadPressed", function (e) {
    ...
    var displayName =
      jqThis.closest(".form-group").find(".axes-config").prop("checked") ||
      jqThis.closest(".form-group").find("input[type=radio]:checked").val() ==
        "dpad"
        ? gpEv.typeName + " " + gpEv.config.choice
        : gpEv.fullname;
    if (basePlayer == gpEv.gamepad) {
      ...
      if (mType == "value") {
        ...
        jqThis.on("GamepadPressed", function (e) {
          ...
          if (
            axEv.config.choiceType == configObj.choiceType &&
            axEv.config.choice == configObj.choice
          ) {
...
function createMapping() {
  ...
  mapGroup
    .find(".map-message")
    .empty()
    .before("<div class='map-message'></div>")
    .remove();
  ...
  mapGroup
    .has('.disable-item:not(:checked) + [data-object=""]')
    .each(function () {
      mapMsg("Please select a button/axis to map", $(this), "error");
    });
  if (
    mapGroup.has('[data-object]:not([data-object=""]), .disable-item:checked')
      .length == 0
  )
    return {};
  var localMapping = {};
  ...
  mapGroup
    .has('[data-object]:not([data-object=""]), .disable-item:checked')
    .each(function () {
      ...
      var obTarget = $(this)
        .find("select[name=" + obtType + "]")
        .val();
      var cInfo = $(this)
        .find("button[data-button=positive]")
        .attr("data-object");
      var sendObj = {
        targetType: obtType,
        target: obTarget,
      ...
      if (
        fixAxes &&
        $(this).find('.fix-axes [data-value]:not([data-value=""])').length == 2
      ) {
        sendObj.axesConfig = {
          type: $(this).find(".fix-axes select").val(),
          lowValue: $(this)
            .find(".fix-axes [data-button=low-value]")
            .attr("data-value"),
          highValue: $(this)
            .find(".fix-axes [data-button=high-value]")
            .attr("data-value"),
        };
      } else if (fixAxes) {
      ...
      if (obtType == "buttons") {
        ...
        var cInfo2 = $(this)
          .find("button[data-button=negative]")
          .attr("data-object");
...
function inputToggle(type, number, gamepad) {
  if (!tester.ifDisabledExists(type, gamepad, number)) {
    ...
    tester.DISABLED_INPUTS[gamepad][type] =
      tester.DISABLED_INPUTS[gamepad][type] || {};
...
function changeCssURL(cssURL) {
  ...
  var re2 =
    "(gist\\.github\\.com|gist\\.githubusercontent\\.com|raw\\.githubusercontent\\.com)"; // Fully Qualified Domain Name 1
...
var messages = [
  'Have you been to our Discord before? We want <a href="https://forms.gle/qhKAKSdxnMQwcR2e8" target="_blank">your feedback</a>!',
];

var messagesOld = [
  'Do you create skins? We want <a href="https://forms.gle/TgqirKrzvV14YBnu5" target="_blank">your feedback</a>!',
  'GPV is getting an upgrade soon and <a href="https://forms.gle/TgqirKrzvV14YBnu5" target="_blank">needs your feedback</a>!',
  ...
  'Sharing is caring, so <a href="https://twitter.com/intent/tweet?hashtags=gpv&text=Shout%20out%20to%20%40GamepadViewer!%20This%20thing%20is%20the%20bits%20for%20showing%20a%20controller%20on%20stream!%20%F0%9F%8E%AE&url=https%3A%2F%2Fgamepadviewer.com" target="_blank">tweet about the Gamepad Viewer!</a>',
  'Join the <a href="https://discord.gg/0SdzYaRROBqfdd0v" target="_blank">Discord Server</a> if you\'re looking for help or want to chat with others!',
];
...
if (pnumber !== "" && $.inArray(pnumber, allowedPlayers !== -1)) {
  ...
  if (
    controlType !== "" &&
    typeof allowedControllers[controlType] !== "undefined"
  ) {
    switchClass(
      "#gamepads .controller",
      "xbox",
      allowedControllers[controlType]
    );
  }
  $("html, body").css({
