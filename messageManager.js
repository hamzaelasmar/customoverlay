...
const isValidOrigin = (origin) => {
    ...
    const allowedHostnames = [
        ...
        /^app\.gpv\.gg$/,
        /^gamepadviewer\.com$/,
        /^beta\.gamepadviewer\.com$/,
        /^gamepaddisplay\.com$/,
        /^stg\.gamepaddisplay\.com$/,
        /^([^.]+?\.)*frontend-staging-d1j\.pages\.dev$/,
        /^([^.]+?\.)*frontend-3qc\.pages\.dev$/
    ];

    const isAllowedOrigin = allowedHostnames.some((h) =>
        h.test(originUrl.hostname)
    );
...
const isValidMessage = (message) => {
    const validations = {
        ...
        "Command Check":
            message.command &&
            Object.keys(controlCodes).includes(message.command),
    };

    const isValid = Object.keys(validations).every(
